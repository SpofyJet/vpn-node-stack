...
node_conntrack_plan() {
    ...
    # CONFIG_NF_CONNTRACK_HELPER ядра — на части конфигов (баг на prod-ноде
    # 2026-09-22) файла нет даже после modprobe nf_conntrack, и sysctl -p
    # убивает весь apply. Отключаем best-effort в runtime — см. node_conntrack_apply.
...
node_conntrack_apply() {
    [ "${DRY_RUN:-0}" = "1" ] && return 0
    node_conntrack_ensure_module
    # helper=0 best-effort: sysctl зависит от CONFIG_NF_CONNTRACK_HELPER ядра
    # (на части конфигов файла нет даже после modprobe) — НЕ фатален. С ядра 4.7
    # helper'ы и так не привязываются автоматически, ключ лишь запечётетвает.
    if [ -e /proc/sys/net/netfilter/nf_conntrack_helper ]; then
        sysctl -w net.netfilter.nf_conntrack_helper=0 >/dev/null 2>&1 \
            || log warn "conntrack" "nf_conntrack_helper=0 не применился (busy/lock) — не фатально"
    else
        log info "conntrack" "nf_conntrack_helper sysctl отсутствует (CONFIG_NF_CONNTRACK_HELPER=n) — helper'ы уже off по умолчанию с ядра 4.7"
    fi
